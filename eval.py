...
import torch
import json
from my_evaluator import myGPT_Evaluator

import time
root_path = '/mnt/share/xujing/nuaa_nlp/Chinese-LLaMA-Alpaca/scripts/ceval/'
choices = ["A", "B", "C", "D"]
...
def main(args, evaluator,take):
    ...
    filenames = os.listdir(os.path.join(root_path, "data/val"))
    ...
    for index,subject_name in enumerate(subject_list):
        ...
        val_file_path=os.path.join(root_path, 'data/val',f'{subject_name}_val.csv')
        dev_file_path=os.path.join(root_path, 'data/dev',f'{subject_name}_dev.csv')
        test_file_path=os.path.join(root_path, 'data/test',f'{subject_name}_test.csv')
