...
class myGPT_Evaluator(Evaluator):
    def __init__(self, choices, k, model_path, device, temperature=0.2):
        super(myGPT_Evaluator, self).__init__(choices, model_path, k)
        load_type = torch.float16
        self.model_path = model_path
        self.device = device
        self.tokenizer = Tokenizer("/mnt/share/xujing/nuaa_nlp/model/vocab.txt", min_occur_cnt=1, specials=[])
        # self.model = myGPT.from_pretrained(
        #     model_path,
        #     load_in_8bit=False,
        #     torch_dtype=load_type,
        #     low_cpu_mem_usage=True,
        #     device_map='auto')
        self.model = myGPT(local_rank=0, 
    ...
    def eval_subject(self, subject_name,
        ...
        for row_index, row in tqdm(test_df.iterrows(), total=len(test_df)):
            ...
            inputs, _ = s2t(instruction, self.tokenizer)
            # print(inputs)
            ...
            #     )
            generation_output = self.model.work(inputs.cuda())
            # batch_size, length = inputs.input_ids.shape
            length = len(inputs)
            print(generation_output[0].shape)
            if constrained_decoding is True:
                # logits = generation_output.scores[0][0]
                logits = generation_output[1][0]
                # print(logits)
